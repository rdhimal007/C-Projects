...
public:
  ...
  //destroys the as it moves the pointer
  virtual ~Grid() {
    for (int i = 0; i < NumRows; i++) {
      CELL* cur = Rows[i];
      CELL* prev = nullptr;
  ...
  Grid(const Grid<T>& other) {
    ...
    //loops until the nuber of rows, inserts the new cell
    for (int i = 0; i < NumRows; i++) {
  ...
  Grid& operator=(const Grid& other) {
    ...
    // clear the memory of prevoius data
    clear();
    Rows = new CELL*[other.NumRows];
    ...
    CELL* cur = nullptr;
    CELL* thisOther = nullptr;
    // loops and inserts the elemnets of the other
    for (int i = 0; i < NumRows; i++) {
      Rows[i] = new CELL(nullptr, other.Rows[i]->Val, other.Rows[i]->NumCols);
      cur = Rows[i];
      thisOther = other.Rows[i];
  ...
  T& operator()(size_t r, size_t c) {
    ...
    int i = 0;
    // loops until it reach the pointer
    while (i < c) {
      cur = cur-> Next;
      i++;
    }
